...
public class FrameProduto extends JFrame {
	...
	public FrameProduto() {
		...
		tbProduto = new JTable(
				new DefaultTableModel(new Object[][] {}, new String[] { "ID", "Nome", "Preço", "Marca" }) {
		...
		tbProduto.getSelectionModel().addListSelectionListener(e -> {
			if (!e.getValueIsAdjusting() && tbProduto.getSelectedRow() != -1) {
				int selectedRow = tbProduto.getSelectedRow();

				Long id = (Long) tbProduto.getValueAt(selectedRow, 0);
				String nome = (String) tbProduto.getValueAt(selectedRow, 1);
				Double preco = (Double) tbProduto.getValueAt(selectedRow, 2);
				String marca = (String) tbProduto.getValueAt(selectedRow, 3);

				tfIdProduto.setText(String.valueOf(id));
				tfNomeProduto.setText(nome);
				tfPrecoProduto.setText(String.valueOf(preco));
				cbMarcas.setSelectedItem(marca);
				...
				try {
					Marca marcaSelecionada = ((ProdutoDaoHib) produtoDao).buscarMarcaPorNome(marca);
					produto.setMarca(marcaSelecionada);
				} catch (Exception ex) {
					JOptionPane.showMessageDialog(FrameProduto.this, "Erro ao buscar marca: " + ex.getMessage(), "Erro",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	...
	private void carregarMarcasNoComboBox() {
		try {
			ProdutoDaoHib produtoDao = new ProdutoDaoHib(EMFactory.getEntityManager());
			List<Marca> marcas = produtoDao.listarMarcas();
